...
pub trait CodegenVisitorTrait<'a> {
    ...
    */
    ...
    fn visit_program(&mut self, program: &Program, codegen: &mut Codegen<'a>) -> Result<()>;
    fn visit_func(&mut self, func: &Func, codegen: &mut Codegen<'a>) -> Result<()>;
    fn visit_statement(&mut self, stmt: & Statement, codegen: &mut Codegen<'a>) -> Result<()>;
    //fn visit_guard(& self, label: &Option<IdTy>, guard: & Guard) -> Result<()>;
    fn visit_guard(&mut self, guard: & Guard, codegen: &mut Codegen<'a>) -> Result<()>;
    fn visit_expr(&mut self, expr: &Expr, codegen: &mut Codegen<'a>) -> Result<()>;
    fn visit_term(&mut self, term: &Term, codegen: &mut Codegen<'a>) -> Result<()>;
    fn visit_struct(&mut self, stru: &Struct, codegen: &mut Codegen<'a>) -> Result<()>;
